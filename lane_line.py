...
def draw_lines(img, lines, roi_height, color=[ 255, 0, 0], thickness=10):
    global SAVED_LB, SAVED_LT, SAVED_RB, SAVED_RT, ACTIVE, ALARM, threshold
    left_x = []
    left_y = []
    right_x = []
    right_y = []
    ...
    if lines is None:
        if SAVED_LB == None or SAVED_LT == None or SAVED_RB == None or SAVED_RT == None:
            if ALARM > threshold:
                ALARM = ALARM + 1
                return -2
            ALARM = ALARM + 1
    ...
    left_distance = (SAVED_LT-SAVED_LB)
    right_distance = (SAVED_RB-SAVED_RT)

    ''' print ("left: "+str(left_distance), flush=True)
    print ("right: "+str(right_distance), flush=True)
    print(" ", flush=True) '''
    middle_t = (SAVED_RT - SAVED_LT)/2 + SAVED_LT
    middle_b = width/2

    cv2.line(img, (SAVED_LB, bot_y),(SAVED_LT, top_y), color, thickness)
    cv2.line(img, (SAVED_RB, bot_y),(SAVED_RT, top_y), color, thickness)
    cv2.line(img, (int(middle_b), bot_y),(int(middle_t), top_y), [0, 255, 0], thickness)

    if middle_t - middle_b > 15:
        return 1
    elif middle_b - middle_t > 15:
        return -1
    else:
        return 0
...
def hough_lines(img, rho, theta, threshold, min_line_len, max_line_gap, roi_height):
    ...
    signal = draw_lines(line_img, lines, roi_height)
    return line_img, signal
...
def process_image(image):
    ...
    # add hough transform
    lines, signal = hough_lines(edges, rho, theta, threshold, min_line_length, max_line_gap, h_pad)
